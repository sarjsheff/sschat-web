...
Клиент мессенджера [sschat](https://github.com/sarjsheff/sschat) - self-hosted
сервера чата на Go со сквозным шифрованием.

Сквозное шифрование, лента с виртуальным окном, SSE-стрим событий, офлайн-кеш в
IndexedDB. Ванильные Web Components + Lit, без роутера и стейт-менеджера.
Собирается как веб-приложение и как desktop/iOS-шелл через Tauri 2.
...
Нужен работающий [sschat](https://github.com/sarjsheff/sschat) - один бинарник
на Go со встроенной базой, внешних сервисов не требует:

```sh
JWT_SECRET=$(openssl rand -hex 32) go run .   # в каталоге sschat, слушает :8080
```
